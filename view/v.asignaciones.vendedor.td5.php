...
# Informacion de la tabla de Comentarios para buscar el estado de la venta y la fecha proxima
	...
	$result_ecrm_comentarios_v1 = mysql_query("SELECT * FROM ecrm_comentarios_v WHERE id_seguimiento = '$asignacion_vendedor' ORDER BY id_comentarios_v desc limit 0,1");
...
<div class="btn-group btn-group-xs">
	...
	<?php
		if ($row_result_ecrm_comentarios_v1['estadodeventa'] == 'Caliente' OR $row_result_ecrm_comentarios_v1['estadodeventa'] == 'caliente') {
	...
	<button type="button" class="btn btn-default btn btn-<?= $CRM_estadodeventa ?>">
		<span style="text-transform:capitalize;">
			<?= $row_result_ecrm_comentarios_v1['estadodeventa'] ?>
<?php
	if ($_SESSION['form-filters-status'] == $row_result_ecrm_comentarios_v1['estadodeventa']) {
