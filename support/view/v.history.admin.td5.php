...
<?php
	...
	if ($filtrarporvendedor == '') {
		$query = "SELECT * FROM ecrm_comentarios_s ORDER BY fecharespuesta desc limit 0,100";
		echo '<pre>' . $query .'</pre>';
	}
	else {
		$query = "SELECT * FROM ecrm_comentarios_s WHERE usuario = '$filtrarporvendedor' ORDER BY fecharespuesta desc limit 0,100";
		echo '<pre>' . $query .'</pre>';
	}
