...
$buscador = $_SESSION['buscador'];
  ...
  if ($fecha_inicio_filtro == '--') {
      ...
      $select .= " LIMIT $init, $limit_end";
      # echo '<pre style="width:100%; height:50px; background-color:#111; color:#fff;">'. $where .'</pre>';
      # echo '<pre style="width:100%; height:50px; background-color:#111; color:#fff;">'. $select .'</pre>';
      # </DEFAULT>
      }
...
/* FACTURADO */
$result = mysql_query("SELECT * FROM ecrm_comentarios_v WHERE estadodeventa = 'facturado' ORDER BY fechaproxima desc limit 1000");
...
/* FACTURADO */
      ...
      # DEFAULT
      $count="SELECT COUNT(*) FROM ".TABLE." WHERE usuario != 'perlA' AND asignadoa != 'trash' AND email != 'automail@portal-cosmos.com AND ($where) '";
      $select = "SELECT *FROM ".TABLE." WHERE usuario != 'perlA' AND asignadoa != 'trash' AND email != 'automail@portal-cosmos.com' AND ($where) ORDER BY $order";
      $select .= " LIMIT $init, $limit_end";
       # echo '<pre style="width:100%; height:50px; background-color:#111; color:#fff;">'. $where .'</pre>';
       # echo '<pre style="width:100%; height:50px; background-color:#111; color:#fff;">'. $select .'</pre>';
    }
